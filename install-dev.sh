...
# that zip's themes/), and it's also what you run directly if you have a
# local checkout/build (a git clone with themes/ generated, e.g. via
# ./scripts/update-from-upstream.sh). If you just want to grab and install
# the latest release without cloning anything, use ./install.sh instead --
# it downloads a release zip and hands off to a copy of this script.
#
...
#   ./install-dev.sh --system        install user-level components system-wide instead (needs sudo)
#   ./install-dev.sh --with-plymouth also install the Plymouth boot theme (needs sudo, rebuilds initramfs)
#   ./install-dev.sh --help          show this text
